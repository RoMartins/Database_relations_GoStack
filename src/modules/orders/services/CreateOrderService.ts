...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const availableProductsId = productsExists.map(product => product.id);

    const checkInexistentsProducts = products.filter(
      product => !availableProductsId.includes(product.id),
    );

    if (checkInexistentsProducts.length) {
      throw new AppError(`Product ${checkInexistentsProducts[0].id} not found`);
    }
